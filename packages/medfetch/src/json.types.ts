import type { Extension, FhirResource } from "fhir/r4";
import type { kdvParser } from "./json.parse";

/**
 * Brand type for json object texts
 */
type JSONObjectText<V> = string & { _tag: "json_object"; __value: V };

/**
 * Brand type for json arrays
 */
type JSONArrayText<V> = string & { _tag: "json_array"; __value: V }; // Assume stringified array

/**
 * Removes optionality from keys, and unions the defined value type
 * with null if (and only if) the key-value type also unions `undefined`.
 * Otherwise it's non nullable
 */
...
/**
 * Take out all extension keys from the object
 */
type DefaultKeyFilter<T> = {
    [K in keyof T as K extends `_${string}` ? never : K]: T[K];
};

type FilterKeys<Obj> = DefaultKeyFilter<OptionalToNullable<Obj>>;

/**
 * Filter extension keys out and turn optional keys -> | null
 */
export type Normalize<Obj> = {
    [Key in keyof FilterKeys<Obj>]: FilterKeys<Obj>[Key];
};

/**
 * @internal 
 */
export type Scalar<Obj> = {
    [K in keyof Obj]: Obj[K] extends (infer U)[] | null
        ? JSONArrayText<U> | null
        : Obj[K] extends object
          ? JSONObjectText<Obj[K]> | null
          : Obj[K];
};

/**
 * Any from resourceType -> its children -> their values
 */
export type PathMap = {
    [R in ResourceType]?: {
        [K in keyof Normalize<ResourceFromType<R>>]?:
            | string
            | number
            | boolean
            | JSONArrayText<any>
...
/**
 * Merge in the replacement types from Override to Fallback
 */
export type Merge<
  Fallback,
  Override extends {
...
> = {
  [K in keyof Fallback]: K extends keyof Override
    ? (
        Fallback[K] extends object
          ? Override[K] & Omit<Fallback[K], keyof Override[K]>
          : Override[K]
      )
    : Fallback[K];
};
