...
import type { kdvParser } from "./data.parse";
 ...
 * ```
 */
type ResourceFromType<ResourceType extends FhirResource["resourceType"]> =
    Extract<FhirResource, { resourceType: ResourceType }>;
...
type ValueKeyType<K extends string> = K extends `value${infer T}`
    ? T
    : never;
...
export type StructKey = ValueKeyType<
    Exclude<
        keyof Extension,
        PrimitiveKey | KeyFilter<PrimitiveKey>
    >
>;
