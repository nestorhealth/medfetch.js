...
flowchart LR
    ...
    Z[FHIR API] --> A
```
Medfetch.js will hook into your database of choice to provide an HTTP fetch
...
(this one will run on the actual [binary]()!). Hit run to run the query against the [HealthIT R4 Sandbox Server]().
