...
## What is Medfetch.js?
Medfetch.js is a Javascript implementation of View the [SQL-on-FHIR]() v2 specs. 
...
| Runtime  | Javascript|SQLite  | Postgres | MySQL |
|----------|-----------|--------|----------|-------|
| Browser  | ✅        | ✅     | ❌       |   ❌
| Node.js  | ✅        | ❌     | ❌       |   ❌
...
on the Web Assembly build of SQLite. Hit run to see it Medfetch.js in action!
...
Currently, it is stable on the modern web browsers and [Node.js](https://nodejs.org/en).
...
How you insert the flattened data into a database, if any at all, is up to you.
...
| **Relational Semantics**   | ✅ Can perform JOINs (what else were you expecting!) | ❌ Can only manipulate one resource type at a time. |
| **Transformation Control** |⚠️  SQL for  JSON data transformations + a limited [`sof`]() port on the database. | ✅ Best control over transformations. |

The only tangible downside to the in-memory runner is the implicit requirement of FHIR knowledge, since the 
...
FHIRpath transformations are actually faster in-memory than in-database <sup>1</sup>. Even then, the I/O overhead of
...
<Callout type="info"> 
<sup>1</sup> 
WebAssembly databases may be the exception to this rule, since Medfetch.js can use the in-memory
runner prior to writing the data to the database with native js!
</Callout>
...
something Medfetch.js makes it easy to do no matter which runner you choose!
...
Once you're up and running, feel free to check out the [guides](/guides) page for your particular
view runner. You don't need to read it all to start your FHIR to SQL pipeline, but if you're curious 
about the *why* behind the *what* with Medfetch.js it'll be well worth your time.
