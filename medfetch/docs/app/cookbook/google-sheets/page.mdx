...
their `reasonReference`, and then `INNER JOIN` them so we get a nice compact table
ready for analysis.
...
3rd party users, then you don't need to set up redirects like here and just write
to the Sheets API immediately with your client secret like we do here.
