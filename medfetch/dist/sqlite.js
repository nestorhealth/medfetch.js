...
import { isPrimitiveType, isSystemType, View } from "./schema";
 ...
 */
export function placeholders(numRows, numColumns) {
 ...
 */
export function flatValues(rows) {
    return rows.flatMap((row) => Object.values(row).map((value) => {
        if (value === null) {
            return null;
        }
        else {
...
export function getColumnType(type) {
    return Match.value(type).pipe(Match.when(isSystemType, (systemType) => {
...
export function processConstraints(columnPath) {
    const constraints = [];
    if (columnPath.path.endsWith(`getResourceKey()`)) {
        constraints.push(`PRIMARY KEY`);
    }
    return pipe(columnPath.tags, Arrayh.reduce(constraints, (acc, tag) => {
        return [...acc, tag.value];
    }), Arrayh.join(" "));
}
export function tableMigration(viewDefinition) {
    return pipe(viewDefinition, View.getColumns, (columns) => Arrayh.reduce(columns, `CREATE TABLE "${viewDefinition.name}" (\n`, (acc, column, index) => {
        acc += `\t"${column.name}"`;
