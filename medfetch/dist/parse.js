...
export function getViewDefinitionQuery(args) {
 ...
 */
const extractDefaultAlias = (path) => {
...
const buildSelect = (tables, columnPaths) => {
    if (tables.length === 0) {
    ...
    const select = [View.Column({ column: [] })];
        ...
        if (joinAliasMap[hd]) {
            ...
            if (parent._tag === "ForEach") {
                ...
                if (joinIndex === -1) {
                    ...
                    return [...acc.slice(0, joinIndex), newForEach, ...acc.slice(joinIndex + 1)];
                }
            }
            else {
                ...
                if (joinIndex === -1) {
                    ...
                    const forEachOrNull = acc[joinIndex];
                    ...
                    return [...acc.slice(0, joinIndex), newForEachOrNull, ...acc.slice(joinIndex + 1)];
                }
            }
        }
        return acc;
...
export function queryToViewDefinition(vtabName, query) {
