...
import { Bundle } from "./schema/index.js";
/**
 ...
 *
 */
const nextLink = (bundle) => Array.findFirst(bundle.link, (link) => link.relation === "next").pipe(Option.map((link) => link.url));
/**
 ...
 */
const get = (url) => Effect.tryPromise(() => fetch(url)).pipe(Effect.flatMap((res) => res.ok
    ? Effect.tryPromise(() => res.json())
    : Effect.fail(new Error(`Response not ok! Status: ${res.status}`))), Effect.flatMap(Bundle.decodeUnknown));
/**
 ...
 */
const pageIterator = (baseUrl, resourceType) => async function* (n, maxPageSize) {
    const upperLimit = n < 0 ? Infinity : n;
    let count = 0;
    const firstPage = await Effect.runPromise(get(`${baseUrl}/${resourceType}?_count=${maxPageSize}`));
    yield firstPage;
 ...
 */
export const pages = (baseUrl, resourceType, n = 100, maxPageSize = 250) => Stream.fromAsyncIterable(pageIterator(baseUrl, resourceType)(n, maxPageSize), (e) => new DataError({ message: String(e) }));
/**
 ...
 */
export const flatResources = (stream) => stream.pipe(Stream.runFold([], (acc, bundle) => pipe(bundle.entry, Array.filterMap((entry) => entry.resource !== undefined ?
    Option.some(entry.resource) : Option.none()), resources => Array.appendAll(acc, resources))));
/**
 * Promised version of `flatResources(pages(...))`;
 * @param params - the pages params
 * @returns a flat array of n resources from the server
 */
export async function nresources(...params) {
    return pipe(pages(...params), flatResources, Effect.runPromise);
}
export function unionKeys(resources) {
