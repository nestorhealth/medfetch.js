...
describe("getTableName", () => {
    ...
    it('Succeeds on any "CREATE TABLE" statement that ends in a semi colon', () => {
        const createTable = `CREATE TABLE foo (
                id text,
                bar real
            );`;
    ...
    it('Succeeds on any "CREATE TABLE" statement that doesn\'t end in a semi colon', () => {
        ...
        expect(getTableName(createTable)).toBe("foo");
    });

    it('Returns the FIRST table name if the querytext includes multiple "CREATE TABLE" statements', () => {
        const createTable = `create table foo (id text); create table bar (id text); create table foobar (foo_id text not null references foo (id), bar_id text not null references bar (id));`;
        expect(getTableName(createTable)).toBe("foo");
    });

    it("Normalizes casing of table names without dquotes", () => {
        const expected = getTableName(`create table foobar (id text);`);
        const camel = getTableName(`create table fooBar (id text);`);
        const capital = getTableName(`create table FooBar (id text);`);
    ...
    it('Returns NULL on non "CREATE TABLE" statements', () => {
        ...
        const update = getTableName(
            "update foo set name = 'bar' where id = 'foobar'",
        );
    ...
    it('Returns NULL on an incomplete "CREATE TABLE" statement', () => {
        ...
        const incompleteMadSpaces = getTableName(
            `create table                                   `,
        );
