import { Option, Array, pipe, String, Schema } from "effect";

export const Key = Schema.Literal(
    "base64Binary",
    "boolean",
    "canonical",
    "code",
 ...
 */
export const createValueKey = <TValueTypeName extends Key>(
    t: TValueTypeName,
): ValueKey<TValueTypeName> => `value${String.capitalize(t)}`;

/**
 * Static 'readonly' list of each `value[x]`
 * key possible with a Primitive Key (typename)
 */
export const valueKeys: readonly ValueKey<Key>[] =
    Key.literals.map(createValueKey);

export const isValueKey = (u: unknown): u is ValueKey<Key> =>
    typeof u === "string" && valueKeys.includes(u as any);
...
export function getValueKey<
    TValuedStruct extends { [valueKey in ValueKey<Key>]?: any },
>(t: TValuedStruct): Option.Option<ValueKey<Key>> {
    return pipe(
        t,
        Object.keys,
        Array.findFirst((key): key is ValueKey<Key> => key.startsWith("value")),
    );
}
export function fromValueKey<TKey extends Key>(valueKey: ValueKey<TKey>): TKey {
    return pipe(
        valueKey,
