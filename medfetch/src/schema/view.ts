...
const _Tag = Schema.Struct({
    ...
    value: Schema.Union(
        Schema.Literal("NOT NULL"),
        Schema.TemplateLiteral("REFERENCES ", Schema.String),
    ),
});
...
const _View_ColumnPath = Data.ColumnPath.pipe(
    Schema.typeSchema,
    Schema.pick("path", "name", "collection"),
    Schema.required,
).pipe(
    Schema.extend(
        Schema.Struct({
...
export interface ColumnPath<
    ...
    TFhirType extends FhirType = FhirType,
> extends Schema.Schema.Type<typeof _View_ColumnPath> {
    name: TName;
    type: TFhirType;
}

const View_ColumnPath: Schema.Schema<ColumnPath> = _View_ColumnPath;
 ...
 */
const _ColumnPath = Schema.transform(
    Schema.typeSchema(Data.ColumnPath),
    View_ColumnPath,
    {
        strict: true,
        decode: ({ tags, type, ...rest }) =>
            pipe(
                tags,
                Array.filter((tag) => isTag(tag)),
                Array.dedupeWith((a, b) => a.value === b.value),
                (tags) => ({
                    ...rest,
                    tags,
...
export const ColumnPath: Schema.Schema<ColumnPath, Data.ColumnPath> =
    _ColumnPath;
 ...
 */
export function decodeSelect(nd: Data.Select): Node {
...
const discriminateConstant = <TPrimitive extends Primitive.Key>(
    t: TPrimitive,
) =>
    Data.Constant.pipe(
        Schema.pick(Primitive.createValueKey(t), "name"),
        Schema.required,
        Schema.attachPropertySignature("_tag", t),
    );

const _Constant = Schema.Union(
    discriminateConstant("boolean"),
    discriminateConstant("base64Binary"),
    discriminateConstant("canonical"),
    discriminateConstant("code"),
    discriminateConstant("date"),
    discriminateConstant("dateTime"),
    discriminateConstant("decimal"),
    discriminateConstant("id"),
    discriminateConstant("instant"),
    discriminateConstant("integer"),
    discriminateConstant("oid"),
    discriminateConstant("string"),
    discriminateConstant("positiveInt"),
    discriminateConstant("time"),
    discriminateConstant("unsignedInt"),
    discriminateConstant("uri"),
    discriminateConstant("url"),
    discriminateConstant("uuid"),
);

const ConstantFromData = Data.Constant.pipe((dataSchema) =>
    Schema.transformOrFail(
        Schema.typeSchema(dataSchema),
        Schema.typeSchema(_Constant),
        {
            strict: true,
            decode: (data, _options, ast) => {
                return pipe(
                    data,
                    Primitive.getValueKey,
                    Option.match({
                        onNone: () =>
                            ParseResult.fail(
                        ...
                        onSome: (key: Primitive.ValueKey) => {
                            ...
                            const parsed: typeof _Constant.Type = {
...
export const Constant: Schema.Schema<Constant, Data.Constant> =
    ConstantFromData;

export const getConstantValue = <TConstant extends Constant>(
    constant: TConstant,
) => {
...
const ViewDefinitionFromData = Schema.transform(
    Schema.typeSchema(Data.ViewDefinition),
    Schema.typeSchema(_ViewDefinition),
    {
        strict: true,
        decode: ({ name, where, constant, select, ...rest }) =>
            ({
                ...
                constant: Array.filterMap(constant ?? [], (constant) =>
                    decodeConstantOption(constant),
                ),
                where: where ?? [],
            }) as const,
        encode: ({ _tag, ...rest }) => rest,
...
export interface ViewDefinition
    extends Schema.Schema.Type<typeof ViewDefinitionFromData> {}
export const ViewDefinition: Schema.Schema<
    ViewDefinition,
    Data.ViewDefinition
> = ViewDefinitionFromData;
 ...
 */
export const make = D.tagged<ViewDefinition>("Select");
...
export function applyTags<TViewDefinition extends ViewDefinition>(
    vd: TViewDefinition,
) {
    return pipe(
        vd,
        getColumns,
        Array.reduce([] as Data.Where[], (acc, column) => {
...
export function mapColumns<TViewDefinition extends ViewDefinition>(
    viewDefinition: TViewDefinition,
    f: (column: ColumnPath) => ColumnPath,
) {
