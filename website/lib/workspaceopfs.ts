...
export async function saveWorkspaceName(name: string) {
  const root = (await (
    navigator as any
  ).storage.getDirectory()) as FileSystemDirectoryHandle;

  const medfetch = await root.getDirectoryHandle("medfetch", { create: true });
  const workspaces = await medfetch.getDirectoryHandle("workspaces", {
    create: true,
  });
...
export async function listWorkspaceNames(): Promise<
...
> {
  ...
  try {
    const workspaces = await root
      ...
      .then((d: { getDirectoryHandle: (arg0: string) => any }) =>
        d.getDirectoryHandle("workspaces"),
      );

    const results: { name: string; createdAt: number }[] = [];
    for await (const entry of workspaces.values()) {
      if (entry.kind === "directory") {
        const meta = await entry
          .getFileHandle("meta.json")
          .then((fh: { getFile: () => any }) => fh.getFile());
