...
it doesn't touch the internal logic of the database.
...
It is stable on modern web browsers and [Node.js](https://nodejs.org/en).
...
FHIRpath transformations are actually faster in-memory than in-database. But for most 
real-world pipelines, the simpler option is often to run Medfetch.js directly inside the database to save yourself
...
anytime soon. At the same time, we know SQL *definitely* isn't going anywhere.
